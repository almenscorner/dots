...
Rectangle {
    ...
    Process {
        id: cavaProcess
        command: ["cava", "-p", (Directories.home + "/.config/cava/quickshell-media.conf")]
        running: true
        stdout: SplitParser {
            onRead: (line) => {
                const parts = line.trim().split(";").filter(s => s.length > 0)
                if (parts.length > 0) {
                    root.cavaValues = parts.map(v => parseInt(v) || 0)
                }
            }
        }
    }
    ...
    Row {
        ...
        Repeater {
            ...
            Item {
                ...
                width: (cavaVisualizer.width - (root.cavaValues.length - 1) * cavaVisualizer.spacing) / root.cavaValues.length
                ...
                Rectangle {
                    ...
                    anchors.left: parent.left
                    anchors.right: parent.right
                    height: (root.cavaValues[index] / 100) * cavaVisualizer.height
