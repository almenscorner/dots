...
VIDEO_OPTS="no-audio loop-file=inf hwdec=auto scale=bilinear interpolation=no video-sync=desync panscan=1.0 video-scale-x=1.0 video-scale-y=1.0 video-align-x=0.5 video-align-y=0.5 load-scripts=no fps=30"
...
for monitor in \$(hyprctl monitors -j | jq -r '.[] | .name'); do
    mpvpaper -p -o "$VIDEO_OPTS" "\$monitor" "$video_path" &
    sleep 0.1
done
EOF
